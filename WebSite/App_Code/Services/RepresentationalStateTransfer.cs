        ...
        protected virtual void CombineScripts(HttpContext context, bool isSaaS, string scriptName, string culture, string version)
        {
            ...
            if (isSaaS)
            {
                if (ApplicationServices.IsTouchClient)
                    ...
                    sb.AppendFormat(string.Format("$(\'<link></link>\').appendTo($(\'head\')).attr({{ href: \'{0}/App_Themes/qa_portal/_T" +
                ...
                sb.AppendFormat("var __targetFramework=\'4.7.2\';__tf=4.0;__cothost=\'appfactory\';__appInfo=\'qa_porta" +
                        "l|{0}\';", BusinessRules.JavaScriptString(context.User.Identity.Name));
        ...
        protected virtual void BeginResponsePadding(HttpRequest request, StreamWriter sw)
        {
            ...
            else
            {
                if ((request.HttpMethod == "GET") && UriRestConfig.IsJSONPRequest(request))
                {
                    ...
                    sw.Write("qa_portal=typeof qa_portal==\'undefined\'?{{}}:qa_portal;qa_portal.{0}=", instance);
                }
            }
        }
